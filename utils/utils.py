...
def calculate_binomial_parameters(
    volatility, trading_days_left, trading_days, pricing_steps, interest_rate, dividend_yield
):
    ...
    up_branch_probability = (interest_rate_cost_of_step - down_branch_move) / (
        up_branch_move - down_branch_move + epsilon
...
def price_option(
    ...
    for i in range(pricing_steps - 1, -1, -1):
        for j in range(i + 1):
            option_value = (
                up_branch_probability * option_value_tree[i + 1, j]
                + down_branch_probability * option_value_tree[i + 1, j + 1]
            ) * factor_step_discount
    ...
    def objective_function(volatility):
        ...
        up_branch_move, down_branch_move, factor_step_discount, up_branch_probability, down_branch_probability = (
            calculate_binomial_parameters(
                volatility,
                time_to_expiration * TRADING_DAYS,
                TRADING_DAYS,
                pricing_steps,
                interest_rate,
                dividend_yield,
        ...
        calculated_option_price = price_option(
            stock_price_tree,
            option_value_tree,
            pricing_steps,
            up_branch_probability,
