        ...
        for stock_price in stock_prices:
            if position == "long":
                ...
                if option_type == "share_purchase":
                    ...
                    payout = min(
                        option_cost - max(strike_price - stock_price, 0), option_cost
                    )  # This short put bit wasn't fully tested as wasn't part of this section. Should work though!
                    print(f"Put Short: if stock price is {stock_price} then payout is {payout}")
                    payouts[title].append(payout)

        plt.plot(stock_prices, payouts[title], label=f"{title} Option Profit and Loss")

    # Calculate combined payouts dynamically
    combined_payouts = [sum(payout) for payout in zip(*payouts.values(), strict=False)]
