    ...
    def test_deque_and_adx(self):
        ...
        for index, row in self.my_data_frame.iterrows():
            if index == TestAlphaLogic.PERIOD_ONE_LENGTH + 1:
                self.assertEqual(
                    len(self.deque_dictionary["period_one"]),
                    TestAlphaLogic.PERIOD_ONE_LENGTH,
                    "Deque 'period_one' + 1 test failed for period 1",
                )
                self.assertEqual(
                    len(self.deque_dictionary["period_two"]),
                    TestAlphaLogic.PERIOD_ONE_LENGTH + 1,
                    "Deque 'period_one' + 1 test failed for period 2",
                )
                self.assertEqual(
                    len(self.deque_dictionary["period_three"]),
                    TestAlphaLogic.PERIOD_ONE_LENGTH + 1,
                    "Deque 'period_one' + 1 test failed for period 3",
                )
                ...
                self.assertEqual(
                    ...
                    "calculate_adx() should have raised a ValueError",
                )
            elif index == TestAlphaLogic.PERIOD_TWO_LENGTH + 1:
                self.assertEqual(
                    len(self.deque_dictionary["period_one"]),
                    TestAlphaLogic.PERIOD_ONE_LENGTH,
                    "Deque 'period_two' + 1 test failed for period 1",
                )
                self.assertEqual(
                    len(self.deque_dictionary["period_two"]),
                    TestAlphaLogic.PERIOD_TWO_LENGTH,
                    "Deque 'period_two' + 1 test failed for period 2",
                )
                self.assertEqual(
                    len(self.deque_dictionary["period_three"]),
                    TestAlphaLogic.PERIOD_TWO_LENGTH + 1,
                    "Deque 'period_two' + 1 test failed for period 3",
                )
            elif index == TestAlphaLogic.PERIOD_THREE_LENGTH + 1:
                self.assertEqual(
                    len(self.deque_dictionary["period_one"]),
                    TestAlphaLogic.PERIOD_ONE_LENGTH,
                    "Deque 'period_three' + 1 test failed for period 1",
                )
                self.assertEqual(
                    len(self.deque_dictionary["period_two"]),
                    TestAlphaLogic.PERIOD_TWO_LENGTH,
                    "Deque 'period_three' + 1 test failed for period 2",
                )
                self.assertEqual(
                    len(self.deque_dictionary["period_three"]),
                    TestAlphaLogic.PERIOD_THREE_LENGTH,
                    "Deque 'period_three' + 1 test failed for period 3",
                )
            elif index == 52:
                break
        ...
        self.assertEqual(
            calculate_adx(self.deque_dictionary["period_three"]),
            [40.48136255393826, 175.39975020822467, 27.52034622638257, 19.060191841982864],
